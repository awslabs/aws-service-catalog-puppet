...
class CreateTemplateForWorkspaceTask(tasks.PuppetTask):
    ...
    def run(self):
        ...
        service_role = t.Sub("arn:aws:iam::${AWS::AccountId}:role/servicecatalog-puppet/PuppetDeployInSpokeRole")
        ...
        execute_dry_run_terraform['Name'] = constants.EXECUTE_DRY_RUN_TERRAFORM_PROJECT_NAME
        execute_dry_run_terraform['Description'] = execute_dry_run_terraform['Description'].replace("Execute",
                                                                                                    "DRY RUN of Execute")
        ...
        terminate_terraform['Description'] = terminate_terraform['Description'].replace("Execute", "Terminate")
        terminate_build_spec = copy.deepcopy(execute_build_spec)
        terminate_build_spec['phases']['build']['commands'] = ["terraform destroy -auto-approve"]
        ...
        termminate_dry_run_terraform['Name'] = constants.TERMINATE_DRY_RUN_TERRAFORM_PROJECT_NAME
        new_description = termminate_dry_run_terraform['Description'].replace("Execute", "DRY RUN of Terminate")
        ...
        template.add_resource(
            codebuild.Project(
                "TerminateDryRunTerraformProject",
                **termminate_dry_run_terraform
            )
        )

        self.write_output(
            template.to_yaml(), skip_json_dump=True
        )
