...
class ProvisionWorkspaceTask(
    ...
    def api_calls_used(self):
        return {
            f"codebuild.start_build_{self.puppet_account_id}": 0.5,
            f"codebuild.batch_get_projects_{self.puppet_account_id}": 0.5,
        }
    ...
    def run(self):
        ...
        state_file_path = f"s3://sc-puppet-state-{self.puppet_account_id}/workspace/{self.workspace_name}/{self.account_id}/{self.region}.zip"
        with self.hub_client('codebuild') as codebuild:
            ...
            if options.get("Terraform", {}).get("Version"):
                parameters_to_use.append(
                    dict(
                        name="TERRAFORM_VERSION",
                        value=options.get("Terraform", {}).get("Version"),
                        type="PLAINTEXT",
                    ),
                )
        ...
        if len(self.ssm_param_outputs) > 0:
            with self.hub_client('s3') as s3:
                output_bucket = f"sc-puppet-state-{self.puppet_account_id}"
                output_key = f"terraform-executions/{build.get('id').split(':')[1]}/artifacts/outputs.json"
