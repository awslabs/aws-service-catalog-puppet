...
def run_tasks(
    ...
    if is_list_launches:
        if is_list_launches == "table":
            ...
            for filename in glob(f"output/ProvisionProductDryRunTask/**/{cache_invalidator}.json", recursive=True):
        ...
        if is_dry_run:
            ...
            for filename in glob(f"output/TerminateProductDryRunTask/**/{cache_invalidator}.json", recursive=True):
                result = json.loads(open(filename, "r").read())
                table_data.append(
                    [
                        result.get("effect"),
