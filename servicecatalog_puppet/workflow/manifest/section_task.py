...
class SectionTask(tasks.PuppetTask, manifest_mixin.ManifestMixen):
    ...
    def handle_requirements_for_spoke_execution(self, name, details):
        ...
        for depends_on in dependencies:
            if depends_on.get(constants.AFFINITY) in ["account", "account-and-region", "region"]:
                raise Exception(f"{name} has an invalid affinity of {depends_on.get(constants.AFFINITY)} when using spoke execution mode")
            else:
                requirements.append(generate_dependency_task(depends_on, self.manifest_file_path, self.puppet_account_id, None, None, not self.is_running_in_spoke()))
