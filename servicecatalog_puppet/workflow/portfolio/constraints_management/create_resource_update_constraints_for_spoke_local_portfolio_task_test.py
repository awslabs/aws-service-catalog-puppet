...
class CreateUpdateResourceConstraintsForSpokeLocalPortfolioTaskTest(tasks_unit_tests_helper.PuppetTaskUnitTest):
    ...
    def setUp(self) -> None:
        from servicecatalog_puppet.workflow.portfolio.constraints_management import create_resource_update_constraints_for_spoke_local_portfolio_task
        self.module = create_resource_update_constraints_for_spoke_local_portfolio_task
        
        self.sut = self.module.CreateUpdateResourceConstraintsForSpokeLocalPortfolioTask(
            **self.get_common_args(),
            portfolio_task_reference=self.portfolio_task_reference, spoke_local_portfolio_name=self.spoke_local_portfolio_name, account_id=self.account_id, region=self.region, portfolio=self.portfolio, resource_update_constraints=self.resource_update_constraints, portfolio_get_all_products_and_their_versions_ref=self.portfolio_get_all_products_and_their_versions_ref        
        )
