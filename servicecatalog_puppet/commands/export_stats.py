...
def get_codebuild_build_ids(filter, limit):
    ...
    if filter == "none":
        codebuild_build_list += get_codebuild_build_ids_for(constants.FULL_RUN_CODEBUILD_PROJECT_NAME, limit)
        codebuild_build_list += get_codebuild_build_ids_for(constants.SINGLE_ACCOUNT_RUN_CODEBUILD_PROJECT_NAME, limit)
...
def write_csv(build_details):
    ...
    for build in build_details:
        results.append(",".join([build['id'], build['start'].strftime("%d/%m/%Y %H:%M:%S"), build['duration_in_seconds'], build['status']]))
    return "\n".join(results)
