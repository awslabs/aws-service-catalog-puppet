...
def show_pipelines(format):
        ...
        for type, pipelines in pipelines_to_check.items():
            for pipeline_name, pipeline_details in pipelines.items():
                ...
                for execution in executions:
                    trend.append(
                        dict(
                            ...
                            trigger=trigger_look_up.get(execution.get("trigger").get("triggerType"), "N/A"),
                        )
                    )
                ...
                pipelines[pipeline_name] = {
                    "name": pipeline_name,
                    "trigger": trigger_look_up.get(last_execution.get("trigger").get("triggerType"), "N/A"),
    ...
    if format == "json":
        ...
        table_data = [
            ["Type", "Name", "Trigger", "Execution Id","Start Time", "Status", "Last Commit Id", "Last Commit Message", "Duration", "Trend"],
        ]
        for type, pipelines in pipelines_to_check.items():
            for pipeline_name, pipeline_details in pipelines.items():
                table_data.append(
        ...
        for type, pipelines in pipelines_to_check.items():
            for pipeline_name, pipeline_details in pipelines.items():
                ...
                trend = "<br /> ".join([f"{a.get('trigger')}:{a.get('status')[0]}" for a in pipeline_details.get("trend", [])])
