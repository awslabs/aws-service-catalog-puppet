...
def test_get_config_without_a_default_region(mocked_betterboto_client, mocked_get_home_region):
    # setup
    ...
    mocked_betterboto_client.return_value.__enter__.return_value.get_parameter.return_value = mocked_response
    puppet_account_id = ""
...
with such.A('get_config') as it:
    ...
    def test(case, method_to_call, key, expected_result):
        # setup
        with mocker.patch("servicecatalog_puppet.config.get_config") as mocked_get_config:
            from servicecatalog_puppet import config as sut
